document.addEventListener('DOMContentLoaded', function() {
    ...
    function createFamilyTree(data) {
        ...
        function update(source) {
            ...
            node.append('text')
                ...
                .text(d => {
                    const title = d.data.title ? `${d.data.title} ` : '';
                    return `${title}${d.data.first_name} ${d.data.last_name}`;
                });
        }
    ...
    function buildHierarchy(data) {
        ...
        if (data.children && data.children.length > 0) {
            const mainPersonInTree = findMainPersonNode(root, mainPerson.id);
            if (mainPersonInTree) {
                data.children.forEach(child => {
                    mainPersonInTree.children.push({
                        data: child
                    });
                });
            }
    ...
    function findMainPersonNode(node, personId) {
        if (node.data.id === personId) return node;

        if (node.children) {
            for (const child of node.children) {
                const found = findMainPersonNode(child, personId);
