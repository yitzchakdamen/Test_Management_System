...
def get_family(person_id):
    ...
    try:
        ...
        # גיסים שהחותן שלהם הוא החותן של האדם
        brothers_in_law = []
        ...
        # גיסים מצד אבא: אנשים שאבא שלהם הוא החותן של האדם
        brothers_in_law_from_father = []
        if main_person.father_id:
            brothers_in_law_from_father = session.query(Person).filter(
                Person.father_in_law_id == main_person.father_id,
                Person.id != person_id
            ).all()

        # גיסים שהם ילדים של החותן של האדם
        brothers_in_law_from_father_in_law = []
        if main_person.father_in_law_id:
            brothers_in_law_from_father_in_law = session.query(Person).filter(
                Person.father_id == main_person.father_in_law_id,
                Person.id != person_id
            ).all()

        # איחוד כל סוגי הגיסים והסרת כפילויות לפי מזהה
        all_brothers_in_law = {bro.id: bro for bro in
                               brothers_in_law + brothers_in_law_from_father + brothers_in_law_from_father_in_law}
        ...
        result = {
            ...
            "brothers_in_law": [get_person_info(session, bro_id) for bro_id in all_brothers_in_law]
        }
        session.close()
...
if __name__ == '__main__':
    ...
    if not os.path.exists('../../Downloads/templates'):
        os.makedirs('../../Downloads/templates')
